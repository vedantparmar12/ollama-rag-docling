...
export function IndexForm({ onClose, onIndexed }: Props) {
  ...
  // 🆕 Docling VLM Features
  const [useDoclingVLM, setUseDoclingVLM] = useState(true);
  const [enableAudioTranscription, setEnableAudioTranscription] = useState(true);
  const [extractTables, setExtractTables] = useState(true);
  const [extractFormulas, setExtractFormulas] = useState(true);
  const [extractCode, setExtractCode] = useState(true);
  const [classifyFigures, setClassifyFigures] = useState(true);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 3. build index (run pipeline) with ALL OPTIONS + Docling
      await chatAPI.buildIndex(index_id, {
        ...
        // 🆕 Docling VLM Features
        useDoclingVLM: useDoclingVLM,
        enableAudio: enableAudioTranscription,
        extractTables: extractTables,
        extractFormulas: extractFormulas,
        extractCode: extractCode,
        classifyFigures: classifyFigures
      });
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-xs uppercase tracking-wide text-gray-300 mb-1">
            Documents & Media <InfoTooltip text="Supports PDF, Word, PowerPoint, Excel, Markdown, HTML, and Audio files (MP3, WAV, M4A, FLAC)" />
          </label>
          <label
            htmlFor="file-upload"
          ...
          >
            ...
            <span className="text-xs text-gray-400">Drag & Drop documents or audio files here or click to browse</span>
            <input id="file-upload" type="file" accept="application/pdf,.docx,.doc,.pptx,.ppt,.xlsx,.xls,.html,.htm,.md,.txt,.mp3,.wav,.m4a,.flac" multiple className="hidden" onChange={(e)=>setFiles(e.target.files)} />
          </label>
        ...
        {/* 🆕 Docling VLM Features */}
        <AccordionGroup title={<><span>🪄 Docling VLM Features</span> <InfoTooltip text="Advanced document understanding with Granite-Docling-258M. Extracts tables, formulas, code, and transcribes audio. 30x faster than traditional OCR." /></>}>
          <div className="grid grid-cols-2 gap-4">
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-400">VLM Document Processing <InfoTooltip text="Use Vision-Language Model for superior layout, table, and structure understanding (30x faster on scanned PDFs)" size={12} /></span>
              <GlassToggle checked={useDoclingVLM} onChange={setUseDoclingVLM} />
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-400">Audio Transcription <InfoTooltip text="Transcribe audio files (MP3, WAV, etc.) using Whisper Turbo with timestamps" size={12} /></span>
              <GlassToggle checked={enableAudioTranscription} onChange={setEnableAudioTranscription} />
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-400">Extract Tables <InfoTooltip text="Preserve table structure with headers using TableFormer AI (95% accuracy)" size={12} /></span>
              <GlassToggle checked={extractTables} onChange={setExtractTables} />
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-400">Extract Formulas <InfoTooltip text="Recognize and extract mathematical formulas (LaTeX inline and block)" size={12} /></span>
              <GlassToggle checked={extractFormulas} onChange={setExtractFormulas} />
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-400">Extract Code <InfoTooltip text="Preserve code blocks with syntax and indentation" size={12} /></span>
              <GlassToggle checked={extractCode} onChange={setExtractCode} />
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-400">Classify Figures <InfoTooltip text="Identify and classify charts, diagrams, and images" size={12} /></span>
              <GlassToggle checked={classifyFigures} onChange={setClassifyFigures} />
            </div>
          </div>
        </AccordionGroup>
